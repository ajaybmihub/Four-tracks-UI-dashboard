...
const filesToClean = [
  { name: 'AFCAT - 2018.json', year: '2018' },
  { name: 'AFCAT - 2019.json', year: '2019' }
];
...
filesToClean.forEach(fileInfo => {
  ...
  try {
    ...
    console.log(`Successfully cleaned: ${fileInfo.name}`);
  } catch (err) {
    console.error(`Error cleaning ${fileInfo.name}:`, err.message);
  }
});
