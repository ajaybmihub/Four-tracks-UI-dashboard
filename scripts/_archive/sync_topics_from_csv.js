...
// Manual Mappings from CSV Name -> DB exam_name
const MAPPINGS = {
    // Sheet 2
...
async function sync() {
    ...
    const updateExam = async (csvName, range, notConducted) => {
        const dbName = MAPPINGS[csvName] || csvName;
        const cleanRange = range.replace(/\u2013|\u2014/g, "-").trim(); // Handle en-dash/em-dash
        const cleanNotConducted = notConducted.replace(/None|conducted every year|deferred|no notification/gi, "").trim();

        const result = await Topic.updateMany(
            { exam_name: dbName },
            { $set: { year_range: cleanRange, not_conducted: cleanNotConducted } }
        );
        if (result.matchedCount > 0) {
            console.log(`Updated [${dbName}] -> Range: ${cleanRange}, Gaps: ${cleanNotConducted}`);
        } else {
            // console.warn(`No match for [${csvName}] searched as [${dbName}]`);
        }
    };

    // Sheet 2 Manual Extraction (simplified)
    const sheet2Data = [
    ...
    for (const [name, range, gaps] of sheet2Data) {
        await updateExam(name, range, gaps);
    }

    // Sheet 3 Manual Extraction
    const sheet3Data = [
    ...
    for (const [name, range, gaps] of sheet3Data) {
        await updateExam(name, range, gaps);
    }

    // Sheet 4 Manual Extraction
    const sheet4Data = [
    ...
    for (const [name, range, gaps] of sheet4Data) {
        await updateExam(name, range, gaps);
    }
