...
async function run() {
    ...
    // 1. Read the topics from JSON
    const data = JSON.parse(fs.readFileSync('goverment_qb.topics.json', 'utf8'));
    console.log(`Loaded ${data.length} docs from JSON.`);

    // 2. Clear current topics
    const del = await Topic.deleteMany({});
    console.log(`Deleted ${del.deletedCount} old topics.`);

    // 3. Prep data (remove $oid etc)
    ...
    // 4. Insert restored topics
    await Topic.insertMany(cleaned);
    console.log(`Restored ${cleaned.length} topics from JSON.`);

    // 5. Ensure the 21 Tech Track topics are present (re-seed them to be safe)
    const techDel = await Topic.deleteMany({ track_name: 'Tech Track' });
    console.log(`Ensured tech purge: ${techDel.deletedCount} deleted.`);
    ...
    const toInsert = [];
    dsaTopics.forEach(t => toInsert.push({ track_name: 'Tech Track', category: 'DSA — Data Structures & Algorithms', exam_name: t, conducting_body: 'Standard DSA', year_range: 'Topic-Based', sub_topic: [] }));
    domains.forEach(d => toInsert.push({ track_name: 'Tech Track', category: 'Domain-Based', exam_name: d.name, conducting_body: 'Standard Domains', year_range: 'Topic-Based', sub_topic: d.sub }));

    await Topic.insertMany(toInsert);
    console.log(`Re-inserted 21 Tech Track topics.`);
